...
// report clarity. It can be removed with cmake options -DWITH_PAPI=0.
#ifdef WITH_PAPI
#include "papi_cntr.h"
#endif
#include "neuron.h"
...
int main(int argc, char *argv[])
{
	...
	if(argc != 4)
	{
		throw std::runtime_error(
			"Expecting three arguments. Path to the network file, "
	...
	// The input of the network is an image, which is processed by three fully
	// connected neuron layers. The first layer has 784 inputs corresponding
	// to image pixels and 512 outputs, the second layer has both 512 inputs
	// and outputs, and the last layer has 512 inputs and 10 classification
	// outputs correspoding to the numbers 0-9. Each output describes
	// a "confidence" of the network that the input picture contains the
	// respective number.
	...
	hid_t networkFile = H5Fopen(
		networkDataFile.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT
	);
	simpleReadDataset(
		"/model_weights/dense_1/dense_1/kernel:0", networkFile, weight1
	);
	simpleReadDataset(
		"/model_weights/dense_2/dense_2/kernel:0", networkFile, weight2
	);
	simpleReadDataset(
		"/model_weights/dense_3/dense_3/kernel:0", networkFile, weight3
	);
	simpleReadDataset(
		"/model_weights/dense_1/dense_1/bias:0", networkFile, bias1
	);
	simpleReadDataset(
		"/model_weights/dense_2/dense_2/bias:0", networkFile, bias2
	);
	simpleReadDataset(
		"/model_weights/dense_3/dense_3/bias:0", networkFile, bias3
	);
	// We got all the data, file is no longer needed.
	H5Fclose(networkFile);

	// Read the input, we take only one/N pictures instead all dataset, so we
	// need to specify memspace and dataspace.

	// Get size of the tested dataset.
	size_t imageCount;
	hid_t dataFile = H5Fopen(
		testingDataFile.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT
	);

	// Open dataset.
	hid_t dataset = H5Dopen(dataFile, "/image_count", H5P_DEFAULT);

	hid_t status = H5Dread(
		dataset, H5T_NATIVE_ULONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, &imageCount
	);
	printf("%zu\n", imageCount);
	if (status < 0)
	{
...
//############################################################################//
	...
	// Create a new file using the default properties. If file already exist
	// it will be erased and rewritten.
	hid_t file = H5Fcreate(
		outputDataFile.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT
	);
	...
	status = H5Dwrite(
		dset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, output3
	);
	if (status < 0)
	{
...
void simpleReadDataset(const char *datasetName, hid_t file, void *buffer)
{
	...
	hid_t status = H5Dread(
		dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buffer
	);
	if (status < 0)
	{
...
)
{
	...
	for (size_t i = 0; i < neuronCount; i++)
	{
		...
		output[i] = evalNeuron(
			inputSize, neuronCount, input, weight, bias[i], i
		);
	}
}
...
void transpose2D(float *&data, const size_t dimX, const size_t dimY)
{
	// TODO: Step2 - Use this function to provide more effective memory
	// access patern.
